import AlgebraPageHeader from "@/components/AlgebraPageHeader";
import AlgebraSectionBlock from "@/components/AlgebraSectionBlock";
...
export default function AlgebraYearSection({ meta, skills, footer, pageSizes }: Props) {
  ...
  return (
    <>
      {pages.map((pageSkills, pageIdx) => (
        ...
        >
          {pageIdx === 0 && <AlgebraPageHeader meta={meta} />}
          {pageIdx > 0 && (
          ...
          <div className="skills-list space-y-3">
            {pageSkills.map((skill) => (
              <AlgebraSectionBlock key={skill.code} skill={skill} />
            ))}
          </div>
