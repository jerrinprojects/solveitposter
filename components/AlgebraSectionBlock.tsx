...
import AlgebraExercisePreview, {
  HAS_ALGEBRA_PREVIEW,
} from "./AlgebraExercisePreview";
import ImageCard from "./ImageCard";

interface Props { skill: PosterSkill; }

/* Display skill code with " · " separator to match solveit's
 * `formatLevelId` ("0.4" → "0 · 4"). */
function formatLevelId(id: string) {
  return id.replace(".", " · ");
}

export default function AlgebraSectionBlock({ skill }: Props) {
  const hasPreview = HAS_ALGEBRA_PREVIEW.has(skill.code);
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 print-card">
      ...
      {skill.imageUrl ? (
        ...
        <AlgebraExercisePreview code={skill.code} />
      ) : (
