...
import {
  SameAPProblemPage, buildSameAPProblems, WorksheetVersion,
} from "@/components/SameAPWorksheet";
import type { SameAreaPerimeterLevelSpec } from "@/data/lengthLevels";
...
export default function SameAPWorksheetFrame({
  level, version,
}: {
  level: SameAreaPerimeterLevelSpec; version: WorksheetVersion;
}) {
  const pool = level.pool();
  const all = buildSameAPProblems(pool, version, PER_PAGE * 2);
  ...
  return (
    <>
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        <Page>
          <PosterHeader
            section={`Solveitmaths · Length · Same A/P · ${versionLabel}`}
            title={<>{level.fullId} <CaveatAccent>Same A or Same P</CaveatAccent></>}
