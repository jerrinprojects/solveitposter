...
import {
  QuadProblemPage, buildQuadProblems, WorksheetVersion,
} from "@/components/QuadWorksheet";
import type { QuadLevelSpec } from "@/data/lengthLevels";
...
export default function QuadWorksheetFrame({
  level, version,
}: {
  level: QuadLevelSpec; version: WorksheetVersion;
}) {
  const pool = level.pool();
  const all = buildQuadProblems(pool, version, PER_PAGE * 2);
  const page1 = all.slice(0, PER_PAGE);
  const page2 = all.slice(PER_PAGE, PER_PAGE * 2);
  const versionLabel = `V${version}`;
  const shape = pool[0]?.shape ?? "parallelogram";
  const shapeLabel = shape === "parallelogram" ? "Parallelogram" : "Trapezium";
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        {[1, 2].map((pn) => {
          ...
          return (
            <Page key={`q${pn}`}>
              <PosterHeader
                section={`Solveitmaths · Length · ${shapeLabel} · ${versionLabel}`}
                title={<>{level.fullId} <CaveatAccent>{shapeLabel}</CaveatAccent></>}
