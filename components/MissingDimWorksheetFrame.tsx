...
import {
  MissingDimProblemPage, buildMissingDimProblems, WorksheetVersion,
} from "@/components/MissingDimWorksheet";
import type { MissingDimLevelSpec } from "@/data/lengthLevels";
...
export default function MissingDimWorksheetFrame({
  level, version,
}: {
  level: MissingDimLevelSpec; version: WorksheetVersion;
}) {
  const pool = level.pool();
  const all = buildMissingDimProblems(pool, version, PER_PAGE * 2);
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        {[1, 2].map((pn) => {
          ...
          return (
            <Page key={`q${pn}`}>
              <PosterHeader
                section={`Solveitmaths · Length · Missing Side · ${versionLabel}`}
                title={<>{level.fullId} <CaveatAccent>Missing Side</CaveatAccent></>}
                tagline={<><Highlight>{level.shortTitle}</Highlight>.</>}
              />
              <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
                <MissingDimProblemPage
