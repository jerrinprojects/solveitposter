/* Year Operations year section — mirrors RationalYearSection but routes
 * to YearOps-specific Header/SectionBlock so the preview MAP resolves
 * YearOps codes (1.1–8.17) which overlap with Rational/DecimalPv. */
import YearOpsPageHeader from "@/components/YearOpsPageHeader";
import YearOpsSectionBlock from "@/components/YearOpsSectionBlock";
...
export default function YearOpsYearSection({ meta, skills, footer, pageSizes }: Props) {
  ...
  return (
    <>
      {pages.map((pageSkills, pageIdx) => (
        ...
        >
          {pageIdx === 0 && <YearOpsPageHeader meta={meta} />}
          {pageIdx > 0 && (
          ...
          <div className="skills-list space-y-3">
            {pageSkills.map((skill) => (
              <YearOpsSectionBlock key={skill.code} skill={skill} />
            ))}
          </div>
