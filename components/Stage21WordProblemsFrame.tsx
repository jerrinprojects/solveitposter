...
import {
  Stage21WordProblemsPage, buildStage21WordProblems,
...
export default function Stage21WordProblemsFrame({ version }: { version: WorksheetVersion }) {
  const all = buildStage21WordProblems(version);
  const pages = [
        ...
        {pages.map((problems, idx) => {
          ...
          return (
            <Page key={`q${pageNumber}`}>
              ...
              <PosterFooter rightLabel={`Multiplication · Stage 2.1 Word · ${versionLabel} · Page ${pageNumber}`} />
            </Page>
          );
        })}
        ...
        {pages.map((problems, idx) => {
          ...
          return (
            <Page key={`a${pageNumber}`}>
              ...
              <PosterFooter rightLabel={`Multiplication · Stage 2.1 Word · ${versionLabel} · Page ${pageNumber} Answers`} />
            </Page>
          );
        })}
