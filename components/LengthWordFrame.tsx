...
import { buildLengthWordProblems, filterByVersion } from "@/data/lengthLevels";
...
const POSTER_H = 741;
const PER_PAGE = 6;
const TOTAL = PER_PAGE * 4; // 24 problems across 4 pages
...
export default function LengthWordFrame({
  level, version,
}: {
  ...
  const all = buildLengthWordProblems(pool, version, TOTAL);
  const pages = [
          ...
          return (
            <Page key={`q${pageNumber}`}>
              <PosterHeader
                ...
                tagline={<>Read carefully — <Highlight>find the {operation} and answer</Highlight>.</>}
              />
