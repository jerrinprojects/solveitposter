...
import {
  CompositeProblemPage, buildCompositeProblems, WorksheetVersion,
} from "@/components/CompositeWorksheet";
...
export default function CompositeWorksheetFrame({
  level, version,
}: {
  level: CompositeShapeLevelSpec; version: WorksheetVersion;
}) {
  const pool = level.pool();
  const all = buildCompositeProblems(pool, version, PER_PAGE * 2);
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        {[1, 2].map((pn) => {
          ...
          return (
            <Page key={`q${pn}`}>
              <PosterHeader
                section={`Solveitmaths · Length · Composite · ${versionLabel}`}
                title={<>{level.fullId} <CaveatAccent>Composite Area</CaveatAccent></>}
                tagline={<><Highlight>{level.shortTitle}</Highlight>.</>}
              />
              <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
                <CompositeProblemPage
