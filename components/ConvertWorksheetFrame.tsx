...
import {
  ConvertProblemPage, buildConvertProblems, WorksheetVersion,
} from "@/components/ConvertWorksheet";
import type { ConvertLevelSpec } from "@/data/lengthLevels";
...
export default function ConvertWorksheetFrame({
  level, version,
}: {
  level: ConvertLevelSpec; version: WorksheetVersion;
}) {
  const pool = level.pool();
  const all = buildConvertProblems(pool, version, PER_PAGE * 2);
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        {[1, 2].map((pn) => {
          ...
          return (
            <Page key={`q${pn}`}>
              <PosterHeader
                section={`Solveitmaths · Length · Convert · ${versionLabel}`}
                title={<>{level.fullId} <CaveatAccent>Convert Units</CaveatAccent></>}
              ...
              <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
                <ConvertProblemPage
                  ...
                  heroTitle="Convert the Units"
                  heroSubtitle={level.diagramTagline}
                  cols={COLS} rows={ROWS}
                />
              </div>
