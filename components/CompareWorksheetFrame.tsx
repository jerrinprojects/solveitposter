...
import {
  ComparePairProblemPage, buildComparePairs, WorksheetVersion,
} from "@/components/CompareWorksheet";
import type { ComparePairLevelSpec } from "@/data/lengthLevels";
...
export default function CompareWorksheetFrame({
  level, version,
}: {
  level: ComparePairLevelSpec; version: WorksheetVersion;
}) {
  const pool = level.pool();
  const all = buildComparePairs(pool, version, PER_PAGE * 2);
  ...
  return (
    <>
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        <Page>
          <PosterHeader
            section={`Solveitmaths · Length · Compare · ${versionLabel}`}
            title={<>{level.fullId} <CaveatAccent>Compare Areas</CaveatAccent></>}
            tagline={<><Highlight>{level.shortTitle}</Highlight>.</>}
          />
          <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
            <ComparePairProblemPage
        ...
        <Page>
          <PosterHeader
            section={`Solveitmaths · Length · Compare · ${versionLabel}`}
            title={<>{level.fullId} <CaveatAccent>Compare Areas</CaveatAccent></>}
            tagline={<>Keep going — <Highlight>more pairs</Highlight>.</>}
          />
          <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
            <ComparePairProblemPage
