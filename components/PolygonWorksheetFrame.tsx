...
import {
  PolygonProblemPage, buildPolygonProblems, WorksheetVersion,
} from "@/components/PolygonWorksheet";
import { filterPolygonByVersion } from "@/data/lengthLevels";
import type { PolygonLevelSpec } from "@/data/lengthLevels";
...
export default function PolygonWorksheetFrame({
  level, version,
}: {
  level: PolygonLevelSpec; version: WorksheetVersion;
}) {
  const fullPool = level.pool();
  const filtered = filterPolygonByVersion(fullPool, version);
  const pool = filtered.length > 0 ? filtered : fullPool;
  const all = buildPolygonProblems(pool, version, PER_PAGE * 2);
  ...
  return (
    <>
      ...
      <main style={{ background: "#fff3e6", minHeight: "100vh", paddingBottom: 48 }}>
        ...
        <Page>
          <PosterHeader
            section={`Solveitmaths · Length · Polygons · ${versionLabel}`}
            title={<>{level.fullId} <CaveatAccent>Polygons</CaveatAccent></>}
        ...
        <Page>
          <PosterHeader
            section={`Solveitmaths · Length · Polygons · ${versionLabel}`}
            title={<>{level.fullId} <CaveatAccent>Polygons</CaveatAccent></>}
            tagline={<>Keep going — <Highlight>more polygons</Highlight>.</>}
          />
          <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
            <PolygonProblemPage
